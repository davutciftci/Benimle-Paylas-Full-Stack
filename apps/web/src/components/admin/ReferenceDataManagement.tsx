...
const SECTION_CONFIG: Record<SectionKey, { title: string; label: string; placeholder: string }> = {
    specialties: { title: 'Uzmanlık Alanları', label: 'Yeni uzmanlık alanı', placeholder: 'Örn. Klinik Psikoloji' },
    degrees: { title: 'Mezuniyet Dereceleri / Bölüm', label: 'Yeni derece veya bölüm', placeholder: 'Örn. Lisans, Yüksek Lisans' },
    titles: { title: 'Uzman Ünvanları', label: 'Yeni ünvan', placeholder: 'Örn. Uzman Klinik Psikolog' },
    therapeuticApproaches: { title: 'Çalışma Ekolleri', label: 'Yeni çalışma ekolü', placeholder: 'Örn. BDT, Psikanaliz' },
};

export const ReferenceDataManagement: React.FC = () => {
    const [specialties, setSpecialties] = useState<ReferenceItem[]>([]);
    const [degrees, setDegrees] = useState<(ReferenceItem & { description?: string | null })[]>([]);
    ...
    const handleCreate = async (section: SectionKey) => {
        ...
        try {
            if (section === 'specialties') {
                ...
                const res = await referenceApi.createDegree(newDegree.trim(), newDegreeDesc.trim() || undefined);
                if (res.success && res.data) {
                    setDegrees(prev => [...prev, res.data!].sort((a, b) => a.name.localeCompare(b.name)));
                    setNewDegree('');
                    setNewDegreeDesc('');
                ...
                if (res.success && res.data) {
                    setTitles(prev => [...prev, res.data!].sort((a, b) => a.name.localeCompare(b.name)));
                    setNewTitle('');
                ...
                if (res.success && res.data) {
                    setApproaches(prev => [...prev, res.data!].sort((a, b) => a.name.localeCompare(b.name)));
                    setNewApproach('');
    ...
    const handleDelete = async (section: SectionKey, id: number) => {
        ...
        try {
            ...
            if (res.success) {
                if (section === 'specialties') setSpecialties(prev => prev.filter(x => x.id !== id));
                else if (section === 'degrees') setDegrees(prev => prev.filter(x => x.id !== id));
                else if (section === 'titles') setTitles(prev => prev.filter(x => x.id !== id));
            ...
            <div key={section} className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                    <input
                        ...
                        onKeyDown={e => e.key === 'Enter' && handleCreate(section)}
                    />
                    {extraInput}
                    <button
                        type="button"
                ...
                <ul className="space-y-2">
                    {items.length === 0 && <li className="text-gray-400 text-sm font-medium">Henüz kayıt yok.</li>}
                    {items.map(item => (
                        <li key={item.id} className="flex items-center justify-between gap-2 py-2 px-3 bg-gray-50 rounded-xl">
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
                ...
                <p className="text-gray-500 mt-1 font-medium">Uzmanlık alanları, çalışma ekolleri, bölüm ve ünvanlar. Uzmanlar kendi profillerinde bu listelerden seçim yapar.</p>
            </div>
            {message && (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {renderSection('degrees', degrees, newDegree, setNewDegree, (
                    <input
                        type="text"
                        value={newDegreeDesc}
                ...
                {renderSection('therapeuticApproaches', approaches, newApproach, setNewApproach)}
            </div>
        </div>
    );
};
