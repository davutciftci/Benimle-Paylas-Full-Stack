      ...
      <div className="text-primary font-nunito w-full flex bg-white py-12" id="sss">
        <div className="w-full max-w-3xl mx-auto px-4">
          <div className="text-center mb-8 space-y-2">
            <h1 className='text-2xl md:text-3xl font-bold' style={{ color: '#00435a' }}>Sıkça Sorulan Sorular</h1>
            <p className='text-sm' style={{ color: '#00435a', opacity: 0.8 }}>Hizmetlerimiz hakkında merak ettiğiniz her şey burada.</p>
          </div>
            ...
            {items.map((item, index) => (
              <div key={index} className="mb-1 border border-gray-200 rounded overflow-hidden">
                ...
                >
                  ...
                  {openIndex === index ? (
                    <Minus className="flex-shrink-0 ml-3" size={16} style={{ color: '#00435a', opacity: 0.7 }} />
                  ) : (
                    <Plus className="flex-shrink-0 ml-3" size={16} style={{ color: '#00435a', opacity: 0.7 }} />
                  )}
                </button>
                {openIndex === index && (
                  <div className="p-3 border-t border-gray-200" style={{ backgroundColor: '#f4f4f4' }}>
                    <p className="leading-relaxed text-xs" style={{ color: '#00435a', opacity: 0.9 }}>{item.answer}</p>
                  </div>
                )}
              </div>
            ))}
