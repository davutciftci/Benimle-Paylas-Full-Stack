      ...
      <div className="text-primary font-nunito w-full flex bg-white py-12" id="sss">
        <div className="w-full max-w-3xl mx-auto px-4">
          <div className="text-center mb-8 space-y-2">
            <h1 className='text-2xl md:text-3xl font-bold text-gray-900'>Sıkça Sorulan Sorular</h1>
            <p className='text-sm text-gray-600'>Hizmetlerimiz hakkında merak ettiğiniz her şey burada.</p>
          </div>
            ...
            {items.map((item, index) => (
              <div key={index} className="mb-1 border border-gray-200 rounded overflow-hidden">
                <button
                  ...
                  className="flex items-center justify-between w-full p-3 font-normal bg-white text-gray-900 hover:bg-gray-50 transition-colors text-left"
                >
                  <span className="text-sm font-medium">{item.question}</span>
                  {openIndex === index ? (
                    <Minus className="flex-shrink-0 ml-3 text-gray-500" size={16} />
                  ) : (
                    <Plus className="flex-shrink-0 ml-3 text-gray-500" size={16} />
                  )}
                </button>
                {openIndex === index && (
